...
static int llama_sample_dist(llama_token_data_array * cur_p, llama::rdrand_engine & rng) {
...
struct llama_sampler_dist {
    const uint32_t seed;       // keep for API compatibility
    uint32_t       seed_cur;   // keep for API compatibility 

    llama::rdrand_engine rng;  // replace std::mt19937 with our rdrand_engine
};
...
struct llama_sampler * llama_sampler_init_dist(uint32_t seed) {
    ...
    return new llama_sampler {
        ...
        /* .ctx   = */ new llama_sampler_dist {
            /* .seed     = */ seed,
            /* .seed_cur = */ seed_cur,
            /* .rng           = */ llama::rdrand_engine(seed_cur),
        },
    };
}
